...
public class RequestMessageHandler(IBus bus) : IMessageHandler<RequestMessage>
{
    ...
    {
        AnsiConsole.MarkupLine($"{Colors.Apply($"[class/message/{nameof(RequestMessage)}] : ", "grey")}{Colors.Apply($"id = '{Markup.Escape(message.Id.ToString())}'", HandlerType.ClassMessage)}");
